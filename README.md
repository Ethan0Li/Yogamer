...
- **Architecture Depth** - I utilize the standard 3-layer depth since the number of inputs doesn't increase (132)
...
- **Baseline** - Quick to classify random states as poses with 1.00 confidence (viewing of my face was Butterfly Pose)
- **Wider** - Fixes issues of Baseline, but classifies me getting into the pose as the pose. Not much of an issue currently due to the 4s timer, but would be problematic in the future when poses are similar. 
- **Lower LR** - Reverts back to Baseline. This is probably due to overfitting of the training data. 
- **Lower DR** - Reverts back to Baseline. The lower DR allows model to establish specific pathways for classification. 
